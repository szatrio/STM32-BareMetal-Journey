...
int main(void) {
    ...
    // track last tick timestamp for non-blocking delay
    ...
    while (1) {
        ...
        // non-blocking timer: toggle led automatically every 500ms via SysTick
        if ((SysTick_GetTick() - last_heartbeat) >= 500) {
			last_heartbeat = SysTick_GetTick(); // update timestamp
			GPIOA->ODR ^= (1U << LED_PIN);       // toggle led
		}
    }
}
