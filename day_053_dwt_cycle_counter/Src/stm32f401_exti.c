...
void EXTI_PC13_Init(void) {
    ...
    // --- TAMBAHKAN BARIS INI ---
    // Enable EXTI15_10 interrupt in NVIC (IRQ 40, handled in ISER[1])
    NVIC->ISER[1] |= (1UL << (EXTI15_10_IRQn - 32));
}
...
void EXTI15_10_IRQHandler(void) {
    if (EXTI->PR & EXTI_PR_PR13) {
        // 1. Clear Pending Flag FIRST (Write 1 to clear)
        // Prevents duplicate ISR execution caused by peripheral bus write latency
        EXTI->PR = EXTI_PR_PR13;

        // 2. Set event flag for main loop processing
        g_btn_event_flag = 1;
    }
}
