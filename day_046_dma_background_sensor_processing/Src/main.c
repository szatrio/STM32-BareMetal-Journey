...
#define DMA_BUF_SIZE 1 // single channel DMA buffer

/* System & Sensor Calibration Constants */
#define VREF             3.3f   // STM32 ADC voltage reference
#define ADC_MAX_VAL      4095.0f // 12-bit ADC Maximum Resolution (2^12 - 1)
...
int main(void) {
    ...
    UART_Print("\r\n=== DMA NON-BLOCKING BACKGROUND SENSOR PROCESSING ===\r\n");
    ...
    while (1) {
    	...
    	if (DMA_GetTCFlag(DMA2, 0)) {
    		...
    		// Getting data from RAM buffer safely
    		uint16_t raw_adc = adc_dma_buffer[0];

			// Convert raw digital value to physical voltage (0.0V - 3.3V)
			float voltage = ((float)raw_adc / ADC_MAX_VAL) * VREF;

			// Convert to physical sensor scale / percentage (0% - 100%)
			uint8_t percentage = (uint8_t)(((float)raw_adc / ADC_MAX_VAL) * 100.0f);

			char log_buf[100];
			snprintf(log_buf, sizeof(log_buf),
					"[SENSOR TELEMETRY] RAW: %4u | Voltage: %.2fV | Position: %3u%%\r\n",
					raw_adc, voltage, percentage);
			UART_Print(log_buf);
